...
while colors_string:
    ...
    first_color = colors_string.popleft()
    second_color = colors_string.pop() if colors_string else ""
    ...
    for color in (first_color + second_color, second_color + first_color):
        ...
        if len(first_color) > 1:
            colors_string.insert(len(colors_string) // 2, first_color[:-1])
        if len(second_color) > 1:
            colors_string.insert(len(colors_string) // 2, second_color[:-1])
